...
응시자가 두 가지 이상의 상충하는 가치나 책임 사이에서 어려운 결정을 내려야 하는 상황을 제시합니다. (예: 개인의 사생활 vs. 조직의 긴급한 요구, 단기적 성과 vs. 장기적 관계, 원칙 준수 vs. 유연한 대응). 지문은 개인적, 조직적 갈등을 중심으로 서술하고, 선택지는 이러한 딜레마를 해결하기 위한 다양한 접근 방식을 제시해야 합니다.`
    ...
    {
        name: "문서 분석 및 적용 (Document Analysis)",
        instruction: `**문제 유형: 문서 분석 및 적용**
지문에 짧은 가상의 문서(예: 사내 공지, 고객 민원 이메일, 언론 보도자료 초안, 신규 사업 계획서 요약, 안전 규정 일부)를 포함시킵니다. 질문은 해당 문서를 이해하고, 그 내용에 기반하여 가장 적절하게 후속 조치를 취하는 것을 평가해야 합니다. 선택지는 문서의 핵심 내용을 잘못 해석했거나, 중요도를 잘못 판단한 행동을 포함해야 합니다.`
    },
    {
        name: "리더십 및 관리 챌린지 (Leadership/Management Challenge)",
        instruction: `**문제 유형: 리더십 및 관리 챌린지**
팀 관리 상황에 초점을 맞춥니다. (예: 팀원 간의 갈등 중재, 저성과자 면담 및 동기부여, 새로운 업무 방식 도입에 대한 팀원들의 저항 관리, 부서 간의 비협조적인 문제 해결). 질문은 리더로서 이 상황을 어떻게 해결할 것인지를 묻고, 선택지는 다양한 리더십 스타일(예: 지시적, 위임적, 민주적, 설득적)을 반영해야 합니다.`
    },
    {
...
4.  **주제 확장**: 문제 상황은 단순히 역사 내 시설 고장이나 열차 운행 문제에만 국한되지 않도록 하십시오. 다음의 다양한 주제를 적극적으로 활용하여 문제의 폭을 넓혀 주십시오.
    *   **인사/조직**: 팀원 갈등, 성과 관리, 동기 부여, 조직 문화, 부서 이기주의.
    *   **경영/기획**: 신규 사업 타당성 검토, 예산 배정 관련 갈등, 대외 협력 및 계약 문제, 회사 이미지 관련 홍보/언론 대응.
    *   **고객 서비스**: 악성 민원 대응, 서비스 품질 개선 방안, 고객 데이터 기반 정책 수립.
    *   **윤리/청렴**: 불공정한 업무 지시, 내외부 청탁 및 압력, 예산의 부적절한 사용 의심 상황.
5.  **문제 형식 및 난이도**:
    *   **지문 (난이도: 쉬움)**: 600자에서 800자 사이의 한글로 된, **어렵지 않은 평범한 업무 상황**을 제시합니다. 지문 자체는 전문 지식 없이 이해할 수 있도록 평이한 단어와 명확한 문장으로 서술해야 합니다.
